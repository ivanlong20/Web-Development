...
app.get("/filter", (req, res) => {
  const foundJoke = jokes.filter((joke) => joke.jokeType === req.query.type);
  res.json(foundJoke);
});
...
app.put("/jokes/:id", (req, res) => {
  jokes.find((joke) => joke.id === parseInt(req.params.id)).jokeType =
    req.body.type;
  jokes.find((joke) => joke.id === parseInt(req.params.id)).jokeText =
    req.body.text;
...
app.delete("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  jokes.splice(jokes.indexOf(jokes.find((joke) => joke.id === id)));
...
app.delete("/jokes/all", (req, res) => {
  req.query.key === masterKey ? (jokes.length = 0) : res.status(404);
  res.json(jokes);
});
